...
public struct ReasoningConfig: Sendable, Equatable {
    ...
    ///
    /// Declared by the Gemma 4 model types through ``ChatConventionsProviding`` rather
    /// than inferred from `model_type`, and shared here because four model types
    /// (text, unified, LLM and VLM entry points) spell the same protocol.
    ///
